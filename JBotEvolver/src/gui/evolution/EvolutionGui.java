...
public class EvolutionGui extends Gui {
	...
	private class UpdateEvolutionThread extends Thread{
		...
		private void updateETA() {
			...
			double completePercentage = generation/(double)evo.getPopulation(0).getNumberOfGenerations();
			...
			while(true) {
				if(evo != null) {
					while(!evo.isEvolutionFinished()) {
						...
						int evoPopulation = evo.getPopulation(0).getNumberOfCurrentGeneration();
						if(enablePreview && evoPopulation > 0 && evoPopulation != currentGenerationNumber){ // this is to show best individual in run panel
							currentGenerationNumber = evoPopulation;
							if(evo.getPopulation(0).getBestChromosome() != null) {
...
//								jBotEvolver.setChromosome(robots, evo.getPopulation().getBestChromosome());
								ArrayList<Robot> robots = jBotEvolver.createRobots(sim, evo.getPopulation(0).getBestChromosome());
								sim.addRobots(robots);
								//System.out.println("Robot position view window" + sim.getRobots().get(0).getPosition());
