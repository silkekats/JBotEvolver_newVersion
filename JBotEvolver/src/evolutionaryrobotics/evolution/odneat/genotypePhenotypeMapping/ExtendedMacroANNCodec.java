...
import evolutionaryrobotics.evolution.odneat.controlsystem.behaviours.BehaviourDecoder;
import evolutionaryrobotics.evolution.odneat.controlsystem.components.ActionSynapse;
import evolutionaryrobotics.evolution.odneat.controlsystem.components.ArbitratorMacroNeuron;
...
import evolutionaryrobotics.evolution.odneat.controlsystem.components.OutputNeuronWithPriorities;
import evolutionaryrobotics.evolution.odneat.controlsystem.components.PrioritySynapse;
...
import evolutionaryrobotics.evolution.odneat.controlsystem.components.TemplateSynapse;
...
//import statistics.ControllerSaver;
import datastructures.MacroTreeNode;

public class ExtendedMacroANNCodec extends GPMapping<MacroGenome, MacroNetwork> {

	private double minConnectionWeight, maxConnectionWeight;
	//protected final ActivationFunction function = new ActivationSteepenedSigmoid();

	public ExtendedMacroANNCodec(Arguments args) {
		super(args);
		if(args != null){
			minConnectionWeight = args.getArgumentAsDoubleOrSetDefault("minweight", -5);
			maxConnectionWeight = args.getArgumentAsDoubleOrSetDefault("maxweight", 5);
		}
	}
	...
	public MacroNetwork decode(MacroGenome genome) {
		...
		if(this.SERIALISE_CONTROLLERS){
			serialise(genome, net);
		}
		
		return net;
	}

	private void serialise(MacroGenome genome, MacroNetwork net) {
		String id = genome.getId();
//		ControllerSaver instance = ControllerSaver.getInstance();
//		//save genome
//		instance.saveGenome(genome, id, outputDirectory);
//		instance.saveController(net, id, outputDirectory);
	}
		...
		for (int i = 0; i < neurons.size(); i++) {
			...
			if (neuron.getType() == Neuron.OUTPUT_NEURON) {
				if (neuron.getNeuronDepth() == -1) {
					...
					|| neuron.getType() == Neuron.ARBITRATOR_MACRO_NEURON
