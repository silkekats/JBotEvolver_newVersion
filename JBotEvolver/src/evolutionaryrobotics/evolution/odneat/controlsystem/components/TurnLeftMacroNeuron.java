...
public class TurnLeftMacroNeuron extends ProgrammedMacroNeuron implements Serializable{

	protected String macroId;
	protected final int UNITS_LEFT_MOVEMENT;
	protected double priority, actionLeft, actionRight;
	
	protected boolean executing = false;
	protected int timesExecuted = 0;
	
	protected final double LEFT_ACTION = 0;
	protected final double RIGHT_ACTION = 0.1;
	
	public TurnLeftMacroNeuron(long innovationNumber, String macroId, int unitsMovement){
		this.innovationNumber = innovationNumber;
		this.macroId = new String(macroId);
		this.UNITS_LEFT_MOVEMENT = unitsMovement >= 1 ? unitsMovement : 1;
	...
	protected double getActivationValue(long synapseId) {
		...
		String value = this.outgoingConnections.get(synapseId);
		if(value.equals("a1"))
			return actionLeft;
	...
	public void step() {
		if(!this.executing){
			...
			if(priority > 0){
				this.timesExecuted = this.UNITS_LEFT_MOVEMENT;
